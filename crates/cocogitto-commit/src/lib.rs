...
pub use conventional_commit_parser::commit::{CommitType, ConventionalCommit, Footer};

pub mod error;

pub fn validate_and_get_message(
    commit_type: &str,
    scope: Option<String>,
    summary: String,
    body: Option<String>,
...
) -> Result<String, Box<ConventionalCommitError>> {
    ...
    let footers = match footers {
        Some(footers) => parse_footers(&footers).map_err(ConventionalCommitError::from)?,
    ...
    conventional_commit_parser::parse(&conventional_message)
        .map_err(ConventionalCommitError::from)?;
...
impl Commit {
    pub fn commit_fixture(commit_type: CommitType, is_breaking_change: bool) -> Self {
        Commit {
            oid: "1234".to_string(),
...
#[cfg(test)]
mod test {
    use crate::{format_summary, verify, Commit};
