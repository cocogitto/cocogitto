use crate::{
    ...
    get_package_changelog_with_target_version, pre_bump_checks, pretty_print_bump_summary,
    run_hooks, tag_or_fallback_to_zero, unwrap_or_stash_and_exit, HookRunOptions,
    PackageBumpOptions,
};
use anyhow::Result;
use cocogitto_bump::bump;
use cocogitto_changelog::get_package_changelog_template;
...
pub fn create_package_version(mut repository: Repository, opts: PackageBumpOptions) -> Result<()> {
    pre_bump_checks(&mut repository, opts.skip_untracked)?;

    let current_tag = repository.get_latest_package_tag(opts.package_name);
    let current_tag = tag_or_fallback_to_zero(current_tag)?;
    let mut next_version = bump(&current_tag, opts.increment, &repository)?;
    ...
    if !disable_bump_commit {
        ...
        if opts.skip_ci || opts.skip_ci_override.is_some() {
            ...
            repository.commit(&format!("chore(version): {tag}"), sign, true)?;
        }
    }
